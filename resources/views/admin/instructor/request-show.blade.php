    ...
    <div class="card shadow-sm mb-4">
        ...
        <div class="card-body">
            <div class="row mb-3">
                ...
                <div class="col-md-6">
                    <p><strong>Phone:</strong> <a href="tel:{{ $instructor->phoneNo }}">{{ $instructor->phoneNo }}</a></p>
            ...
            <div class="row">
                <div class="col-md-6">
                    <p><strong>Status:</strong> 
                        <span class="p-2 badge badge-{{ $instructor->status == 'approve' ? 'success' : ($instructor->status == 'pending' ? 'warning' : 'danger') }}">
                            ...
                            <div class="card h-100">
                                <div class="card-body text-center">
                                    @if(in_array($attachment->file_type, ['image/jpeg', 'image/png', 'image/gif']))
                                        <img src="{{ asset('user-attachments/' . $attachment->file_name) }}" alt="Attachment" class="img-fluid img-thumbnail mb-2" style="max-height: 150px;">
                                    @else
                                        <a href="{{ asset('user-attachments/' . $attachment->file_name) }}" target="_blank" class="btn btn-outline-info">{{ $attachment->file_name }}</a>
                                    @endif
                                    <div class="mt-2">
                                        <select class="form-control attachment-status" data-attachment-id="{{ $attachment->id }}">
                                            <option value="approve" {{ $attachment->file_status == 'approve' ? 'selected' : '' }}>Approved</option>
                                            <option value="pending" {{ $attachment->file_status == 'pending' ? 'selected' : '' }}>Pending</option>
                                            <option value="rejected" {{ $attachment->file_status == 'rejected' ? 'selected' : '' }}>Rejected</option>
                                        </select>
